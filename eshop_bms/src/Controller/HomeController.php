...
class HomeController extends AbstractController
{
    ...
    public function home(TokenStorageInterface $tokenStorage, AuthorizationCheckerInterface $authorizationChecker, EntityManagerInterface $entityManager): Response
    {
        ...
        return $this->render('bms_home/home.html.twig', [
            'shopInfo' => $shopInfo,
            'roles' => $roles,
    ...
    {
        if (!$imageName) {
            return new JsonResponse(['status' => 'Error', 'message' => '未提供文件名'], 400);
        }
        $imageName = $imageName;
        // 获取最新的 ShopInfo 实体
        $shopInfo = $entityManager->getRepository(ShopInfo::class)->findOneBy([], ['id' => 'DESC']);
        if (!$shopInfo) {
            ...
            return new JsonResponse(['status' => 'Error', 'message' => '店铺信息未找到']);
        }
        ...
        if (in_array($imageName, $existingImageUrls)) {
            ...
            if (file_exists($fullFilePath)) {
                ...
                return new JsonResponse(['status' => 'Error', 'message' => '文件未找到']);
            }
    
            // 更新 `carouselPictures` 列
            $imageName = $imageName;
            ...
            return new JsonResponse(['status' => 'Success', 'message' => '文件删除成功']);
        } else {
            $logger->error("3333333333333333333333333");
            return new JsonResponse(['status' => 'Error', 'message' => '文件不在数据库记录中']);
        }
    }
}
