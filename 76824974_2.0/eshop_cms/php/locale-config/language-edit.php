<?php
$html = file_get_contents("./html/backend-content-translation_template.php");
...
if (isset($page["id"])) {
    ...
    $html = str_replace("{{APP_DIRECTORY}}", $config->APP_DIRECTORY, $html);
    for ($i = 0; $i < count($config->ELEMENT_IDS); $i++) {
        $element_id = $config->ELEMENT_IDS[$i];
        ...
        $html = str_replace($config->HTML_TRANSLATION_ELEMENTS_TO_CHANGE[$i], $element_content, $html);
    }
} else {
    $html = str_replace("{{APP_DIRECTORY}}", $config->APP_DIRECTORY, $html);
    foreach ($config->HTML_TRANSLATION_ELEMENTS_TO_CHANGE as $translation_element) {
